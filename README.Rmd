...
If you do not already have R installed, because _**`theme.usq`**_ is an R package,
you will need to install R first. Download and install the proper version for
your computer from [CRAN](https://cran.r-project.org). It is also suggested to
have RStudio installed as well. You can download the proper installation file
from https://www.rstudio.com/products/rstudio/download/ for your platform.

Once R and RStudio are set up, you're ready to install _**`theme.usq`**_ through an
R session.

The _**`theme.usq`**_ package is only available from GitHub. The easiest way to
install it is by using the [_**`devtools`**_](https://github.com/hadley/devtools)
package.
...
Following are a few examples of _theme.usq's_ capabilities. Please see the 
[package vignette](https://adamhsparks.github.io/theme.usq/articles/theme.usq.html)
...
the `cool` palette to use bluish USQ colours for the graph.
...
`scale_fill_usq` to use USQ colours for continuous values in the graph. Two
types of gradients are included, `warm_gradient` and `cool_gradient`.
...
#### Using _**ggplot2**_ and `theme_usq()`
...
Use the default `_**ggplot2**_` colour scheme to fill the density plot while using
...
* Please report any
[issues or bugs](https://github.com/adamhsparks/theme.usq/issues).
