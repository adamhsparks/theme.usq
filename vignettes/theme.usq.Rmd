...
Note the use of `interpolate = TRUE` here, this data set has 15 values (carmakers) but the palette only has 15 colours.
This extends the colours to allow the use of this palette.
Note that here, rather than using `coord-flip()` that "manufacturer" has been mapped to the y-axis.

```{r example-bright}
ggplot(data = mpg, aes(y = manufacturer, fill = manufacturer)) +
  geom_bar() +
  scale_fill_usq(palette = "bright", guide = "none", interpolate = TRUE) +
  theme_usq()
```
